        ...
        const [
            ...
            ctx.db.query("classInstances")
                ...
                .filter((q) =>
                    ...
                    q.gte(q.field("startTime"), currentMonthStart) &&
                    q.lte(q.field("startTime"), currentMonthEnd)
                )
                .collect()
                .then(res => res.length),
            ...
            ctx.db.query("bookings")
                ...
                .filter((q) =>
                    q.eq(q.field("deleted"), undefined) &&
                    q.eq(q.field("status"), "completed") &&
                    q.gte(q.field("bookedAt"), currentMonthStart) && // Using bookedAt or class start time? Usually completion is based on class time, but for simplicity using bookedAt or maybe better to filter by classInstanceSnapshot.startTime if available.
                    q.lte(q.field("bookedAt"), currentMonthEnd)
                )
                .collect()
                .then(res => res.length),
